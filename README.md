...
An intelligent, offline Retrieval-Augmented Generation (RAG) assistant for Old School RuneScape (OSRS). Built with custom PyTorch Lightning embedding models, FAISS vector search, Hugging Face Sentence Transformers and a local Llama 3.1 8B LLM running on Ollama inside Docker.
...
* due to the intially long run times ive left progress bars and other would be debug information active to show progression
