...
def build_rag_prompt(user_query: str, hits: list[dict]) -> str:
    ...
    - Be sure page title contains the main query object text. for example a query with abyssal demon should not return abyssal sire unless the user queried about a boss version.
    - Do not combine adjacent item names or attributes (e.g., do not combine 'Dragon spear' and 'Bones' into 'Dragon bones').
    - Ensure that the exact item requested matches the entity's drop table before making claims about drop rates.
    - Provide a clean, scannable summary answering the core question (e.g., stats, primary BiS gear, core requirements, or exact drop rates). Keep initial summaries concise.
    - Do not dump exhaustive inventory layouts, phase-by-phase mechanics, or low-tier budget substitutes all at once unless explicitly requested.
    - Always end your response by offering 2-3 specific follow-up topics the user can explore next (e.g., Detailed Inventory Setup, Specific Phase Mechanics, Budget Gear Substitutes).
